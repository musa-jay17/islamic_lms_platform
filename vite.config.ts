...
// @ts-ignore - no TypeScript declaration file for '@babel/traverse' in this project
...
function cdnPrefixImages(): Plugin {
  ...
  const rewriteJsxAst = (code: string, id: string, cdn: string) => {
    ...
    traverse(ast, {
      JSXAttribute(path: import('@babel/traverse').NodePath<t.JSXAttribute>) {
      ...
      StringLiteral(path: import('@babel/traverse').NodePath<t.StringLiteral>) {
      ...
      TemplateLiteral(path: import('@babel/traverse').NodePath<t.TemplateLiteral>) {
